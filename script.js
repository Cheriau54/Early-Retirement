...
let db = {
    liquid: JSON.parse(localStorage.getItem('v5_liquid')) || [],
    fixed: JSON.parse(localStorage.getItem('v5_fixed')) || [],
    stocks: JSON.parse(localStorage.getItem('v5_stocks')) || [],
    debt: JSON.parse(localStorage.getItem('v5_debt')) || [],
    settings: JSON.parse(localStorage.getItem('v5_settings')) || { 
        masterCurr: "HKD", goal: 1000000, years: 10, income: 0, incomeCurr: "HKD", inflation: 2.5, stkGrowth: 7.0 
    }
};
...
function initUI() {
    ...
    document.getElementById('master-currency').onchange = async (e) => {
        const newCurr = e.target.value;
        const oldCurr = db.settings.masterCurr;
        // Dynamic Pivot Logic for Goal
        if(rates[newCurr]) {
            db.settings.goal = db.settings.goal / rates[newCurr];
            db.settings.masterCurr = newCurr;
            save();
            await refreshAll(false);
        }
    };

    // Form Submissions with Aggregation Logic
    ...
    document.getElementById('calc-btn').onclick = () => { syncSettings(); refreshAll(false); };
...
async function refreshAll(isInitial) {
    ...
    document.getElementById('target-goal').value = db.settings.goal.toFixed(0);
    ...
    db.liquid.forEach((item, i) => {
        ...
        liqBody.innerHTML += `<tr><td>${item.name}</td><td>${item.amount}</td><td>${mVal.toFixed(0)}</td><td><button onclick="del('liquid',${i})">✕</button></td></tr>`;
    });
    ...
    // EQUITIES
    const stkBody = query('#table-stocks tbody'); stkBody.innerHTML = 'Loading...';
    let stkHtml = '';
    ...
    db.debt.forEach((item, i) => {
        ...
        dbtBody.innerHTML += `<tr><td>${item.name}</td><td class="loss">-${item.amount}</td><td>${mVal.toFixed(0)}</td><td><button onclick="del('debt',${i})">✕</button></td></tr>`;
    });
...
function runAudit(nw, t, mCurr) {
    ...
    // NEW LOGIC: Real Rate of Return (Fisher Equation)
    ...
    document.getElementById('logic-output').innerHTML = `
        NW: ${nw.toLocaleString()} | Nom Yield: ${(nominalYield*100).toFixed(1)}% | Real Yield: ${(realYield*100).toFixed(1)}%
        ------------------------------------------------------------
        1. Projected Wealth (Inflation Adj): ${fv.toLocaleString(undefined,{max:0})} ${mCurr}
        2. Shortfall vs Goal: ${gap.toLocaleString(undefined,{max:0})} ${mCurr}
        3. REQUIRED SAVING/MO: ${monthlyNeeded.toLocaleString(undefined,{max:0})} ${mCurr}
        4. MONTHLY SURPLUS/LOSS: <span class="${surplus>=0?'surplus':'loss'}">${surplus.toLocaleString(undefined,{max:0})} ${mCurr}</span>
    `;
...
function updateCharts(t) {
    ...
    document.getElementById('chart-legend').innerHTML = `
        ...
        <div class="legend-item"><div class="dot" style="background:#facc15"></div> Stocks: ${pStk.toFixed(0)}%</div>
    `;
}
...
function syncSettings() {
    db.settings = { 
        masterCurr: val('master-currency'), goal: fVal('target-goal'), years: fVal('years-to-goal'), 
        inflation: fVal('inflation-rate'), stkGrowth: fVal('stock-growth'),
        income: fVal('monthly-income'), incomeCurr: val('income-curr') 
    };
    save();
}
function save() { 
    localStorage.setItem('v5_liquid', JSON.stringify(db.liquid)); localStorage.setItem('v5_fixed', JSON.stringify(db.fixed));
    localStorage.setItem('v5_stocks', JSON.stringify(db.stocks)); localStorage.setItem('v5_debt', JSON.stringify(db.debt));
    localStorage.setItem('v5_settings', JSON.stringify(db.settings));
}
